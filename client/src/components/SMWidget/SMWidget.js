...
class SMWidget extends Component {
  render() {
    ...
    const padding = (variant === '0' ? { card: 'p-3', icon: 'p-3', lead: 'mt-2' } : (variant === '1' ? {
      card: 'p-0', icon: 'p-3', lead: 'pt-1',
    } : { card: 'p-0', icon: 'p-4 px-5', lead: 'pt-3' }));

    const card = { style: 'clearfix', color: color, icon: icon, classes: '',width: '30px' };
...
SMWidget.propTypes = propTypes;
SMWidget.defaultProps = defaultProps;
